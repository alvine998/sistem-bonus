...
<div class="flex justify-center items-center mt-[50px] px-10">
    <div class="bg-white shadow w-full rounded-md p-4">
        <h2 class="font-semibold text-xl">Tambah Detail Kriteria</h2>
        <form action="{{route('detail.store', $criterion)}}" enctype="multipart/form-data" method="post">
            @csrf
            <div class="mt-10">
                <div class="flex flex-col gap-1">
                    <label for="detail">Detail Kriteria</label>
                    <input id="detail" name="detail" type="text" value="{{old('detail')}}" required placeholder="Masukkan Detail Kriteria" class="w-full p-1 pl-2 rounded border border-gray-300" />
                </div>
                <div class="flex flex-col gap-1 mt-2">
                    <label for="score">Nilai</label>
                    <input id="score" name="score" type="number" value="{{old('score')}}" required placeholder="Masukkan Nilai" class="w-full p-1 pl-2 rounded border border-gray-300" />
                </div>
            </div>
